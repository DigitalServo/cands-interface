...
pub mod driver;
use driver::ftdi::{FtdiDriver, Ft232h, TimeoutError};
...
impl TCAN455xTranceiver {
    ...
    pub fn setup(&mut self, sidf: &[SIDFCONFIG], xidf: &[XIDFCONFIG]) -> Result<(), Box<dyn std::error::Error>>{
    ...
    pub fn reset(&mut self) -> Result<(), Box<dyn std::error::Error>>{
        self.driver.set_pin(0x01)?;
        std::thread::sleep(std::time::Duration::from_millis(5));
        self.driver.set_pin(0x00)?;
        std::thread::sleep(std::time::Duration::from_millis(5));
        Ok(())
    }
